...
function resolveResponse(response, requestedUrl, gifKey, worker) {
  ...
  if (response.status >= 400) {
    onError(response, gifKey, worker, "Something went wrong. Server responded with: Status " + repsonse.status + ", " + response.statusText);
  } else if (!response.json.webmUrl) {
    if (!fileHelper.urlEndsWithDotGif(requestedUrl)) {
      onError(response, gifKey, worker, "Could not convert " + fileHelper.getFileExtension(requestedUrl) + " to gfycat video. Displaying original image");
    } else {
      onError(response, gifKey, worker, "Can not show video. webm video does not exist. Displaying original image");
    }
    //onError(response, gifKey, worker, null);

  } else if (response.json.error) {
    onError(response, gifKey, worker, response.json.error + " Displaying original image");
  } else {
    // Success
...
function onError(response, gifKey, worker, errorMessage) {
  worker.port.emit("gfyInfoFetchError", response.json, gifKey, errorMessage);
}
