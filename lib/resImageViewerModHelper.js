...
function resolveResponse(response, requestedUrl, gifKey, worker) {
  ...
  if (response.status >= 400) {
    ...
    if (!urlHelper.urlEndsWithDotGif(requestedUrl)) {
      onError(response, gifKey, worker, "Could not convert " + urlHelper.getFileExtension(requestedUrl) + " to gfycat video. Displaying original image");
    } else {
